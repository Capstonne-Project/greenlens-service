...
| ------------ | ------------ | ------------------------------------------ | ---------------------------- |
| _(mới tạo)_  | `Assigned`   | Officer gọi `/assign`                      | —                            |
...
| `assignmentStatus` | string | _(tất cả)_ | Lọc: `Assigned`, `InProgress`, `Completed`, `Declined` |
...
| `page`             | int    | 1          | Trang hiện tại                 |
| `pageSize`         | int    | 20         | Số item mỗi trang              |
| `assignmentStatus` | string | _(tất cả)_ | Lọc theo trạng thái assignment |
...
### 4. Chấp nhận task _(bắt buộc trước khi làm việc)_
...
### 5. Từ chối task _(chỉ trong 2h sau khi được giao)_
...
### 6. Cập nhật tiến độ + upload ảnh _(tuỳ chọn, nhiều lần)_
...
```json
{
  ...
  "status": 200,
  "data": {
    "uploadedImageUrls": ["https://cdn.example.com/progress/img1.jpg", "https://cdn.example.com/progress/img2.jpg"]
  }
}
```
