---
name: ecoreport-feature-slice
description: Scaffold a new vertical-slice feature (CQRS Command or Query) in the EcoReport .NET 9 Clean Architecture backend (project SU26SE049). Use this skill whenever the user asks to add a new use case, endpoint, command, query, handler, or feature to the backend — even casual phrasings like "make an endpoint for verifying reports", "add a command to assign cleanup team", "I need a query to fetch nearby reports", or "create the submit report feature". Trigger this even when the user only mentions one piece (e.g. "just write the validator for X") because the slice should stay consistent. Produces the full 4-file slice (Command/Query, Handler, Validator, Response DTO) with proper folder placement under Application/Features/<Module>/<UseCase>/, MediatR + FluentValidation + Result<T> patterns, BR-ID XML comments, and CancellationToken propagation.
---

# EcoReport Feature Slice (CQRS)

This skill scaffolds a single vertical slice for the EcoReport backend (project **SU26SE049 — Crowdsourced Application for Reporting Environmental Pollution**). It enforces the conventions in the workspace `CLAUDE.md` so every feature looks the same.

## When to use

Trigger this skill any time a new use case is added to `EcoReport.Application/Features/`. Even if the user asks for "just a handler" or "just an endpoint", produce the full slice — partial slices drift from the project standard and force rework later.
...
4. **Write each file to its target path** under `src/EcoReport.Application/Features/__MODULE__/__USECASE__/`.
5. **Do NOT also generate the endpoint here.** The endpoint belongs in `EcoReport.Api/Endpoints/` (or `Controllers/` if the project uses controllers — check before assuming). If the user wants the endpoint too, hand off to `ecoreport-controller-base` afterwards.
6. **Stop and ask** if you are about to:
   - Reference an entity, enum, or repository that does not yet exist in `EcoReport.Domain` or `EcoReport.Application/Common/Interfaces/`
...
```
src/EcoReport.Application/Features/__MODULE__/__USECASE__/
...
- Returns `Result<T>` from `EcoReport.Domain.Common.Result` — never throw for business failures, only for infrastructure faults.
...
5. Tell the user: "Slice written. The endpoint isn't included — want me to add it next? That would use the `ecoreport-controller-base` skill."
